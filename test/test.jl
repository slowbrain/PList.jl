...
@test writeplist_string(Dict{Any, Any}()) == "{}"
@test writeplist_string(cell(0)) == "()"
...
@test writeplist_string({"foo" => "bar"}) == "{foo = bar;}"
@test writeplist_string({"one" => 1}) == "{one = 1;}"

@test writeplist_string({1, {2, 3}, 4}) == "(1, (2, 3), 4)"
@test writeplist_string({"colors" => {"red", "green", "blue"}}) == "{colors = (red, green, blue);}"
...
@test readplist_string("{foo = bar;}") == {"foo" => "bar"}
@test readplist_string("{one = 1;}") == {"one" => 1}
